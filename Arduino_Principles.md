...
1. Rosen, Boyle, Cariss & Forchelli (2014) on scaffolded EF: external cognitive scaffolds enable students to practice EF they don't yet have. Both pre-written code and a well-designed AI prompt scaffold are external cognitive scaffolds; which one a student needs depends on their current readiness.
2. Morsink et al. (2022) on ADHD motivation: external reinforcers and answer-givers undermine intrinsic motivation and competence-building. This is why dialogue-level Claude Code use requires the student to describe the problem first — an AI that gives answers on demand becomes an extrinsic reward system; an AI that asks questions back becomes a scaffold. But the same logic cuts the other way: requiring AI dialogue from a student who is not ready for it also undermines competence-building, by setting up a barrier the student cannot cross. The pre-written upload level exists precisely to avoid that failure mode.
3. Smith, Langberg et al. (2020) on adolescent ADHD motivation: extrinsic support helps academic outcomes when structured, not when it replaces the student's own work. Pre-written code is extrinsic support that structures the experience of the hardware working; it does not replace the student's own work on everything else in the project (wiring, testing, debugging the circuit, making choices about what to build next).

**Honest caveat:** This principle is the weakest-supported in the list and is flagged explicitly in the Honest Limitations section of the master document. If the master document is going to argue that Claude Code integration is a valuable feature, it must acknowledge this is an untested practice being piloted in good faith — and specifically, the three-level differentiated model has no precedent in published research and is our own design response to the heterogeneity of the student population.
...
- Students can move up (Level 1 → 2 → 3) within a session or across sessions, and can also move down without shame. A student who is having a hard day can drop from Level 3 to Level 1 for a session and return to Level 3 next week.
- For project 8 (quadcopter), the `TinyQuadcopter.ino` file in the source material is currently an empty stub — the real firmware lives as pseudocode in the README. This is actually a feature: **Level 1** students upload a simpler pre-written motor-test sketch that proves the electronics work (no stabilization). **Level 2/3** students use Claude Code to generate the full PID-stabilized .ino from the README pseudocode. Even within the most complex project in the program, the three-level model applies.
